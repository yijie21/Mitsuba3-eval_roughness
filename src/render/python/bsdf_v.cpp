...
public:
    ...
    Spectrum eval_roughness(const SurfaceInteraction3f &si,
                            Mask active) const override {
        PYBIND11_OVERRIDE(Spectrum, BSDF, eval_roughness, si, active);
    }
...
template <typename Ptr, typename Cls> void bind_bsdf_generic(Cls &cls) {
    ...
    cls.def("sample",
        ...
        .def("eval_roughness",
            [](Ptr bsdf, const SurfaceInteraction3f &si, const Mask &active) {
