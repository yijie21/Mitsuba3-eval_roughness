...
public:
    ...
    SmoothPlastic(const Properties &props) : Base(props) {
        ...
        m_roughness = props.texture<Texture>("roughness", 0.0f);
    ...
    Spectrum eval_roughness(const SurfaceInteraction3f & si, Mask active) const override {
        return m_roughness->eval(si, active);
    }
